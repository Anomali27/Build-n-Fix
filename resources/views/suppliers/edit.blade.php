...
<div class="container mx-auto px-4 lg:px-8 py-8 max-w-3xl">
    ...
    <nav class="flex items-center gap-2 text-xs font-semibold text-gray-500 mb-6">
        ...
        <span class="text-gray-400">&gt;</span>
        <span class="text-[#F97316] font-bold">Edit Pemasok</span>
    </nav>
    ...
    <div class="bg-white rounded-3xl border border-gray-200/80 shadow-sm overflow-hidden">
        <div class="p-6 md:p-8 bg-gray-50 border-b border-gray-100 flex items-center justify-between">
            <div>
                <h1 class="text-xl md:text-2xl font-black text-gray-900">Ubah Data Pemasok</h1>
                <p class="text-xs text-gray-500 mt-1">Perbarui profil kontak atau alamat pemasok {{ $supplier['name'] }}.</p>
            </div>
        ...
        <form action="{{ route('suppliers.update', $supplier['id']) }}" method="POST" class="p-6 md:p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="address" rows="2" 
                              class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl text-xs font-semibold text-gray-900 focus:outline-none focus:ring-2 focus:ring-[#F97316]">{{ old('address', $supplier['address']) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <select name="status" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl text-xs font-semibold text-gray-900 focus:outline-none focus:ring-2 focus:ring-[#F97316]">
                        ...
                        <option value="inactive" {{ ($supplier['status'] ?? '') === 'inactive' ? 'selected' : '' }}>Non-Aktif</option>
                    </select>
                </div>
            </div>
